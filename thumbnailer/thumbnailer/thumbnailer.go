...
// DefaultWarmSize is the Thumbnail size pre-generated by the warm work
// queue (see warm.go) when Options.WarmSize is not set. 512 covers a
// three-column grid on a 1080p phone (roughly 360px per cell, sizes snap
// upward via fitSize) - roughly 320MB for 8,000 photos rather than well
// over a gigabyte for all four ThumbnailSizes. Larger viewer sizes are
// deliberately left on demand, served by the existing interactive path.
const DefaultWarmSize = 512
...
type Options struct {
	...
	MaxImageWidth  int
	MaxImageHeight int
	// WarmSize is the single Thumbnail size pre-generated by the warm work
	// queue (see warm.go). Zero means "use the default" (DefaultWarmSize).
	// Exactly one size is ever warmed - larger viewer sizes stay on demand.
	// Like every other request it snaps upward to a ThumbnailSizes preset
	// (see fitSize), which is what keeps warmed Thumbnails enumerable by
	// invalidateThumbnails; a value between presets warms the next one up.
	WarmSize int
}
...
func NewThumbnailer(p Params, fileProviderId string, path string, thumbnailStorage fileprovider.Client) (Result, error) {
	...
	if options.WarmSize == 0 {
		options.WarmSize = DefaultWarmSize
	}
...
func (t *Thumbnailer) Start() error {
	...
	// the durable warm-request consumer runs alongside the preview request
	// loop too, with its own (smaller) concurrency budget - see warm.go.
	warm, err := t.startWarmConsumer(ctx)
	if err != nil {
		t.log.Error("failed to start thumbnail warm consumer; background pre-generation will not run", "error", err)
	} else {
		t.mu.Lock()
		t.warm = warm
		t.mu.Unlock()
...
func (t *Thumbnailer) Stop() error {
	...
	// waits for messageLoop, any in-flight handleMessage goroutines, the
	// invalidation consumer's loop/handlers, and the warm consumer's
	// loop/handlers, which subsumes the limiters' own accounting of running
	// work
	t.wg.Wait()
