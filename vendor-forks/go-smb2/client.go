...
func (fs *Share) remove(name string) error {
	...
	req := &CreateRequest{
		...
		CreateDisposition:    FILE_OPEN,
		// FILE_OPEN_REPARSE_POINT omitted: Samba returns STATUS_NOT_SUPPORTED
		// for regular files opened with this flag. DELETE access is kept -
		// FileDispositionInformation requires it, and without the reparse-point
		// flag Samba accepts the CREATE (the earlier STATUS_NOT_SUPPORTED was
		// from FILE_OPEN_REPARSE_POINT, not from DELETE).
		CreateOptions: FILE_SYNCHRONOUS_IO_NONALERT,
	}
...
func (fs *Share) Rename(oldpath, newpath string) error {
	...
	create := &CreateRequest{
		...
		ShareAccess:          FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE,
		CreateDisposition:    FILE_OPEN,
		// FILE_OPEN_REPARSE_POINT omitted: Samba STATUS_NOT_SUPPORTED on regular
		// files. FILE_SYNCHRONOUS_IO_NONALERT matches the OpenFile access pattern.
		CreateOptions: FILE_SYNCHRONOUS_IO_NONALERT,
	}
